...
public class PathView extends View {
    ...
    public void setStroke(float stroke) {
        if (stroke < 0.0f || stroke > 1.0f) {
            throw new IllegalArgumentException("setStroke not between 0.0f and 1.0f");
        }
        this.stroke = stroke;
        synchronized (mSvgLock) {
            updatePathsPhaseLocked2();
        }
        invalidate();
    }


    private void updatePathsPhaseLocked2() {
        final int count = paths.size();
        if(count > 0 && stroke > 0F) {
            int tempProgress = (int) (stroke * MAX_VALUE);
            int quotient = MAX_VALUE / count;
            int currentIndex = tempProgress / quotient;
            ...
            for(int index = 0; index<count; index++) {
                SvgUtils.SvgPath svgPath = paths.get(index);
                svgPath.path.reset();

                if(currentIndex == index) {
                    svgPath.measure.getSegment(0.0f, svgPath.length * strokeRatio(tempProgress, quotient, index), svgPath.path, true);
    ...
    private float strokeRatio(float progress, float quotient, int index) {
        return (progress - (quotient * index)) / quotient;
    }
...
//            anim = ObjectAnimator.ofFloat(pathView, "percentage", 0.0f, 1.0f);
            anim = ObjectAnimator.ofFloat(pathView, "stroke", 0.0f, 1.0f);
        }
